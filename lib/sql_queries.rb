...
FROM projects INNER JOIN pledges ON pledges.project_id=projects.id HAVING sum(pledges.amount) > projects.funding_goal GROUP BY projects.title;"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"Write your SQL query Here"
"SELECT users.name, sum(pledges.amount) FROM users INNER JOIN pledges on pledges.user_id=users.id GROUP BY name ORDER BY sum(pledges.amount) ASC, name;"
end
